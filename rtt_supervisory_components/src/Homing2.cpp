...
void Homing::updateHook()
{
    if (send_new_reference == true && homed == false ) {  // if ref changes, the new ref is sent only once to the reference generator
        ...
        send_new_reference = false;
        log(Warning)<< "Homing of " << homing_body << ": send homing reference of joint :" << homing_order_t <<endlog();
        log(Warning)<< "Homing of " << homing_body << ": ref[homing_order_t-1][0] :" << ref[homing_order_t-1][0] <<endlog();
        log(Warning)<< "Homing of " << homing_body << ": ref[homing_order_t-1][1] :" << ref[homing_order_t-1][1] <<endlog();
        log(Warning)<< "Homing of " << homing_body << ": ref[homing_order_t-1][2] :" << ref[homing_order_t-1][2] <<endlog();
    }
