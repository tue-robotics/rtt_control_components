...
#include "AdmittanceControllers.hpp"
...
AdmittanceControllers::AdmittanceControllers(const string& name) :
...
AdmittanceControllers::~AdmittanceControllers()
{
...
bool AdmittanceControllers::startHook()
{
    ...
    if ( !outport.connected() ) {
        log(Warning)<<"AdmittanceControllers::Outputport not connected!"<<endlog();
    }

    if (vector_size < 1 || Ts <= 0.0) {
        log(Error)<<"AdmittanceControllers parameters not valid!"<<endlog();
        return false;
    }
...
ORO_CREATE_COMPONENT(FILTERS::AdmittanceControllers)
