...
TracingTimeStamp::TracingTimeStamp(const string& name)
    : TaskContext(name, PreOperational), filename("data.txt"),
      buffersize(16384), Ts(0.001)
{
  addProperty("vector_sizes", vectorsizes_prop)
      .doc("size of the vector per port. Example: array ( 2.0 4.0 4.0 )");
...
void TracingTimeStamp::updateHook()
{
  ...
  // TODO: if ( !this->isRunning() ) return; (Check if this also works, counter
  // can become a uint
  ...
  for (uint i = 0; i < Nports; i++)
  {
    // get the time
    current_seconds =
        RTT::os::TimeService::Instance()->secondsSince(start_ticks);
    ...
    if (inports[i].read(input) == NewData && counters[i] <= counter)
    {
      ...
      // Fill it with data
      for (uint column = startcolumn; column < startcolumn + vectorsizes[i] - 1;
           ++column)
      {
...
void TracingTimeStamp::stopHook()
{
  ...
  cout << "Trace written!!!! Finished Tracing !!!!!!     End of Tracing Buffer "
          "reached, Controller can be terminated!" << endl;
}
