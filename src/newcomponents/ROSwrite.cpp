...
ROSwrite::ROSwrite(const string& name) : TaskContext(name, PreOperational)
{
	...
	//! Operations
    addOperation("AddAnalogOuts", &ROSwrite::AddAnalogOuts, this, OwnThread)
    ...
    addOperation("AddDigitalOuts", &ROSwrite::AddDigitalOuts, this, OwnThread)
		.doc("Add one or more analog ins")
		.arg("PARTNAME","String specifying the name of the part")
		.arg("INPORT_DIMENSIONS","Array containing for each inport an entry with value the size of that input port")
		.arg("OUTPORT_DIMENSIONS","Array containing for each outport an entry with value the size of that output port")
	...
	//! Add math
	// Analog
	addOperation("AddAddition_A", &ROSwrite::AddAddition_A, this, OwnThread)
	...
	addOperation("AddMultiply_A", &ROSwrite::AddMultiply_A, this, OwnThread)
	...
	addOperation("AddMatrixTransform_A", &ROSwrite::AddMatrixTransform_A, this, OwnThread)
	...
	// Digital
	addOperation("AddForcedHigh_D", &ROSwrite::AddForcedHigh_D, this, OwnThread)
		.doc("This function will force a digital input to be high")
		.arg("PARTNAME","Name of the bodypart")
		.arg("PORTNR","Output port number of the analog out");	
}
ROSwrite::~ROSwrite(){}
...
void ROSwrite::AddAnalogOuts(string PARTNAME, doubles INPORT_DIMENSIONS, doubles OUTPORT_DIMENSIONS, doubles FROM_WHICH_INPORT, doubles FROM_WHICH_ENTRY)
{
	// Init 
	...
	//! Check configuration
	if (!this->isRunning()) {
		log(Error) << "ROSwrite::AddAnalogOuts(" << PARTNAME << "): Could not add AnalogOuts. Since ROSwrite component has not yet been started!" << endlog();
		return;
	}
	...
	for(uint l = 0; l < MAX_BODYPARTS; l++) {
		if (PARTNAME == added_bodyparts_A[l]) {
			log(Error) << "ROSwrite::AddAnalogOuts(" << PARTNAME << "): Could not add AnalogOuts. There is already an AnalogIn for bodypart " << PARTNAME << "!" << endlog();
			return;
		}
	}
	...
	for (uint l = 0; l < bodypart_names.size(); l++) {
		if (bodypart_names[l] == PARTNAME) {
			BPID = l + 1;
			log(Info) << "ROSwrite::AddAnalogOuts(" << PARTNAME << "): BPID IS: " << BPID << "." << endlog();
		}
	}
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSwrite::AddAnalogOuts(" << PARTNAME << "): Could not add AnalogOuts. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}

	// Check for invalid number of ports
	if(N_INPORTS < 1 || N_INPORTS > MAX_PORTS) {
		log(Error) << "ROSwrite::AddAnalogOuts(" << PARTNAME << "): Could not add AnalogOuts. Invalid number of inports: " << N_INPORTS << "!" << endlog();
		return;
	}
	if(N_OUTPORTS < 1 || N_OUTPORTS > MAX_PORTS) {
		log(Error) << "ROSwrite::AddAnalogOuts(" << PARTNAME << "): Could not add AnalogOuts. Invalid number of outports: " << N_OUTPORTS << "!" << endlog();
		return;
	}
	...
	for(uint i = 0; i < N_INPORTS; i++) {
		if(INPORT_DIMENSIONS[i] < 1) {
			log(Error) << "ROSwrite::AddAnalogOuts(" << PARTNAME << "): Could not add AnalogOuts. Inport_dimensions cannot contain value smaller than one!" << endlog();
			return;
		}
	...
	for(uint i = 0; i < N_OUTPORTS; i++) {
		if(OUTPORT_DIMENSIONS[i] < 1) {
			log(Error) << "ROSwrite::AddAnalogOuts(" << PARTNAME << "): Could not add AnalogOuts. Outport_dimensions cannot contain value smaller than one!" << endlog();
			return;
		}
	...
	if ((FROM_WHICH_INPORT.size() != N_OUTPORT_ENTRIES) || (FROM_WHICH_ENTRY.size() != N_OUTPORT_ENTRIES)) {
		log(Error) << "ROSwrite::AddAnalogOuts(" << PARTNAME << "): Could not add AnalogOuts. The number of entries in from_which_inport_A and from_which_entry_A should equal the total number of output values." << endlog();
		return;
	}
	...
	for(uint k = 0; k < N_OUTPORT_ENTRIES; k++) {
		if( FROM_WHICH_INPORT[k] < 0 || FROM_WHICH_INPORT[k] > N_INPORTS ) {
			log(Error) << "ROSwrite::AddAnalogOuts(" << PARTNAME << "): Could not add AnalogOuts. 0 < From_which_inport <= N_INPORTS. -> 0 < " << FROM_WHICH_INPORT[k] << " <= "<< N_INPORTS <<"!" << endlog();
			return;
		}
		else if ( FROM_WHICH_ENTRY[k] < 0 || FROM_WHICH_ENTRY[k] > INPORT_DIMENSIONS[FROM_WHICH_INPORT[k]-1] ) {
			log(Error) << "ROSwrite::AddAnalogOuts(" << PARTNAME << "): Could not add AnalogOuts. From_which_entry array contains entry no. " << FROM_WHICH_ENTRY[k] << " which does not exist for inport no. " << FROM_WHICH_INPORT[k] << "!" << endlog();
			return;
		}
	}
	...
	log(Warning) << "ROSwrite::AddAnalogOuts(" << PARTNAME << "): Added AnalogOuts with " << N_INPORTS << " inports and " << N_OUTPORTS << " outports!" << endlog();

	return;
}

void ROSwrite::AddDigitalOuts(string PARTNAME, doubles INPORT_DIMENSIONS, doubles OUTPORT_DIMENSIONS, doubles FROM_WHICH_INPORT, doubles FROM_WHICH_ENTRY)
{
	// Init 
	uint N_INPORTS = INPORT_DIMENSIONS.size();
	uint N_OUTPORTS = OUTPORT_DIMENSIONS.size();
	...
	//! Check configuration	
	if (!this->isRunning()) {
		log(Error) << "ROSwrite::AddDigitalOuts(" << PARTNAME << "): Could not add DigitalOuts. Since ROSwrite component has not yet been started!" << endlog();
		return;
	}
	// Check if the DigitalOuts is already added for this bodypart
	for(uint l = 0; l < MAX_BODYPARTS; l++) {
		if (PARTNAME == added_bodyparts_D[l]) {
			log(Error) << "ROSwrite::AddDigitalOuts(" << PARTNAME << "): Could not add DigitalOuts. There is already an DigitalOuts for bodypart " << PARTNAME << "!" << endlog();
			return;
		}
	}
	// Check if the bodypart name is in the bodypart_name list and set the BPID accordingly
	for (uint l = 0; l < bodypart_names.size(); l++) {
		if (bodypart_names[l] == PARTNAME) {
			BPID = l + 1;
			log(Info) << "ROSwrite::AddDigitalOuts(" << PARTNAME << "): BPID IS: " << BPID << "." << endlog();
		}
	}
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSwrite::AddDigitalOuts(" << PARTNAME << "): Could not add DigitalOuts. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}
	    
	// Check for invalid number of ports
    if(N_INPORTS < 1 || N_INPORTS > MAX_PORTS) {
        log(Error) << "ROSwrite::AddDigitalOuts(" << PARTNAME << "): Could not add DigitalOuts. Invalid number of inports: " << N_INPORTS << "!" << endlog();
        return;
    }
    if(N_OUTPORTS < 1 || N_OUTPORTS > MAX_PORTS) {
        log(Error) << "ROSwrite::AddDigitalOuts(" << PARTNAME << "): Could not add DigitalOuts. Invalid number of outports: " << N_OUTPORTS << "!" << endlog();
        return;
    }
    ...
    for(uint i = 0; i < N_INPORTS; i++) {
        if(INPORT_DIMENSIONS[i] < 1) {
            log(Error) << "ROSwrite::AddDigitalOuts(" << PARTNAME << "): Could not add DigitalOuts. Inport_dimensions cannot contain value smaller than one!" << endlog();
            return;
        }
        N_INPORT_ENTRIES += INPORT_DIMENSIONS[i];
    }
    for(uint i = 0; i < N_OUTPORTS; i++) {
        if(OUTPORT_DIMENSIONS[i] < 1) {
            log(Error) << "ROSwrite::AddDigitalOuts(" << PARTNAME << "): Could not add DigitalOuts. Outport_dimensions cannot contain value smaller than one!" << endlog();
            return;
        }
        N_OUTPORT_ENTRIES += OUTPORT_DIMENSIONS[i];
    }
	...
	// Check if the total number of entries matches the size of FROM_WHICH_INPORT and FROM_WHICH_ENTRY
    if ((FROM_WHICH_INPORT.size() != N_OUTPORT_ENTRIES) || (FROM_WHICH_ENTRY.size() != N_OUTPORT_ENTRIES)) {
        log(Error) << "ROSwrite::AddDigitalOuts(" << PARTNAME << "): Could not add DigitalOuts. The number of entries in from_which_inport_D and from_which_entry_D should equal the total number of output values." << endlog();
        return;
    }

    for(uint k = 0; k < N_OUTPORT_ENTRIES; k++) {
        if( FROM_WHICH_INPORT[k] < 0 || FROM_WHICH_INPORT[k] > N_INPORTS ) {
            log(Error) << "ROSwrite::AddDigitalOuts(" << PARTNAME << "): Could not add DigitalOuts. 0 < From_which_inport <= N_INPORTS. -> 0 < " << FROM_WHICH_INPORT[k] << " <= "<< N_INPORTS <<"!" << endlog();
            return;
        }
        else if ( FROM_WHICH_ENTRY[k] < 0 || FROM_WHICH_ENTRY[k] > INPORT_DIMENSIONS[FROM_WHICH_INPORT[k]-1] ) {
            log(Error) << "ROSwrite::AddDigitalOuts(" << PARTNAME << "): Could not add DigitalOuts. From_which_entry array contains entry no. " << FROM_WHICH_ENTRY[k] << " which does not exist for inport no. " << FROM_WHICH_INPORT[k] << "!" << endlog();
            return;
        }
    }
	...
	// math statuses
	forcedhigh_status_D[BPID-1].resize(N_INPORTS);
    for( uint i = 0; i < N_INPORTS; i++ ) {
		forcedhigh_status_D[BPID-1][i] = false;
	}
    ...
    log(Warning) << "ROSwrite::AddDigitalOuts(" << PARTNAME << "): Succesfully added DigitalOuts with " << N_INPORTS << " inports and " << N_OUTPORTS << " outports!" << endlog();
...
// Analog
void ROSwrite::AddAddition_A(string PARTNAME, int PORTNR, doubles ADDVALUES)
{
	// init
	uint BPID;	
	...
	//! Check configuration	
	if (!this->isRunning()) {
		log(Error) << "ROSwrite::AddAddition_A([" << PARTNAME << "," << PORTNR << "]): Could not add addition. Since ROSwrite component has not yet been started." << endlog();
		return;
	}
	// Check if the bodypart name is in the bodypart_name list and set the BPID accordingly
	for (uint l = 0; l < bodypart_names.size(); l++) {
		if (bodypart_names[l] == PARTNAME) {
			BPID = l + 1;
			log(Info) << "ROSwrite::AddMultiply_A([" << PARTNAME << "," << PORTNR << "]): BPID IS: " << BPID << "." << endlog();
		}
	}
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSwrite::AddAddition_A([" << PARTNAME << "," << PORTNR << "]): Could not add addition. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}
	
	if( PORTNR <= 0 || PORTNR > n_inports_A[BPID-1]) {
		log(Error) << "ROSwrite::AddAddition_A([" << PARTNAME << "," << PORTNR << "]): Could not add addition. Invalid PORTNR: " << PORTNR << ".  1 <= PORTNR <= " << n_inports_A[BPID-1] << "!" << endlog();
		return;
	}
	if( ADDVALUES.size() != inport_dimensions_A[BPID-1][PORTNR-1] ) {
		log(Error) << "ROSwrite::AddAddition_A([" << PARTNAME << "," << PORTNR << "]): Could not add addition. Invalid size of ADDVALUES " << ADDVALUES.size() << ". Should have been of size :" << inport_dimensions_A[BPID-1][PORTNR-1] << "!" << endlog();
		return;
	}
	if( addition_status_A[BPID-1][PORTNR-1] ) {
		log(Error) << "ROSwrite::AddAddition_A: Could not add addition. For this input a multiplier is already there" << endlog();
	...
	if( addition_status_A[BPID-1][PORTNR-1] ) {
		log(Warning) << "ROSwrite::AddAddition_A([" << PARTNAME << "," << PORTNR << "]): Overwritten existing addition." << endlog();
	} else {
		log(Warning) << "ROSwrite::AddAddition_A([" << PARTNAME << "," << PORTNR << "]): Added addition." << endlog();
	}
...
void ROSwrite::AddMultiply_A(string PARTNAME, int PORTNR, doubles MULTIPLYFACTOR)
{
	// init
	uint BPID;
	...
	//! Check configuration
	if (!this->isRunning()) {
		log(Error) << "ROSwrite::AddMultiply_A([" << PARTNAME << "," << PORTNR << "]): Could not add multiply. Since ROSwrite component has not yet been started." << endlog();
		return;
	}
	// Check if the bodypart name is in the bodypart_name list and set the BPID accordingly
	for (uint l = 0; l < bodypart_names.size(); l++) {
	...
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSwrite::AddMultiply_A([" << PARTNAME << "," << PORTNR << "]): Could not add multiply. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}
	
	if( PORTNR <= 0 || PORTNR > n_inports_A[BPID-1]) {
		log(Error) << "ROSwrite::AddMultiply_A([" << PARTNAME << "," << PORTNR << "]): Could not add multiply. Invalid PORTNR: " << PORTNR << ".  1 <= PORTNR <= " << n_inports_A[BPID-1] << "!" << endlog();
		return;
	}
	if( MULTIPLYFACTOR.size() != inport_dimensions_A[BPID-1][PORTNR-1] ) {
		log(Error) << "ROSwrite::AddMultiply_A: Could not add multiplier. Invalid size of MULTIPLYFACTOR. Should have be of size :" << MULTIPLYFACTOR.size() << "!" << endlog();
		return;
	}
	...
	if( multiply_status_A[BPID-1][PORTNR-1] ) {
		log(Warning) << "ROSwrite::AddMultiply_A([" << PARTNAME << "," << PORTNR << "]): Overwritten existing multiplier." << endlog();
	} else {
		log(Info) << "ROSwrite::AddMultiply_A([" << PARTNAME << "," << PORTNR << "]): Added multiplier." << endlog();
	}
...
void ROSwrite::AddMatrixTransform_A(string PARTNAME, int PORTNR, double INPUTSIZE, double OUTPUTSIZE)
{
	// init
	uint BPID;
	...
	//! Check configuration	
	if (!this->isRunning()) {
		log(Error) << "ROSwrite::AddMatrixTransform_A([" << PARTNAME << "," << PORTNR << "]): Could not add Matrix Transform. Since ROSwrite component has not yet been started." << endlog();
		return;
	}
	...
	for (uint l = 0; l < bodypart_names.size(); l++) {
		if (bodypart_names[l] == PARTNAME) {
			BPID = l + 1;
			log(Info) << "ROSwrite::AddMatrixTransform_A([" << PARTNAME << "," << PORTNR << "]): BPID IS: " << BPID << "." << endlog();
		}
	}
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSwrite::AddMatrixTransform_A([" << PARTNAME << "," << PORTNR << "]): Could not add Matrix Transform. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}
	
	if( matrixtransform_status_A[BPID-1][PORTNR-1] ) {
		log(Warning) << "ROSwrite::AddMatrixTransform_A: Could not add Matrix Transform, since this already excists for this bodypart. Overwriting is not supported at the moment" << endlog();
		return;
	}
	
	if( PORTNR <= 0 || PORTNR > n_inports_A[BPID-1]) {
		log(Error) << "ROSwrite::AddMatrixTransform_A([" << PARTNAME << "," << PORTNR << "]): Could not add Matrix Transform. Invalid PORTNR: " << PORTNR << ".  1 <= PORTNR <= " << n_inports_A[BPID-1] << "!" << endlog();
		return;
	}
	...
	if( INPUTSIZE != inport_dimensions_A[BPID-1][PORTNR-1]) { 
		if( INPUTSIZE < inport_dimensions_A[BPID-1][PORTNR-1]) { 	// smaller matrix then nr of inputs is allowed, not recommended
			log(Warning) << "ROSwrite::AddMatrixTransform_A: INPUTSIZE: " << INPUTSIZE << " is smaller than the size of the inport inport_dimensions_A[BPID-1]:" << inport_dimensions_A[BPID-1][PORTNR-1] << "!" << endlog();
		} else { 										// larger matrix then nr of inputs is not allowed
			log(Error) << "ROSwrite::AddMatrixTransform_A: Could not add Matrix Transform. INPUTSIZE: " << INPUTSIZE << " is larger than the size of the inport inport_dimensions_A[BPID-1]:" << inport_dimensions_A[BPID-1][PORTNR-1] << "!" << endlog();
			return;
		}
	}
	...
	log(Warning) << "ROSwrite::AddMatrixTransform_A([" << PARTNAME << "," << PORTNR << "]): Added a matrix transform." << endlog();
...
void ROSwrite::AddForcedHigh_D(string PARTNAME, int PORTNR)
{
	// init
	uint BPID;
	...
	//! Check configuration	
	if (!this->isRunning()) {
		log(Error) << "ROSwrite::AddForcedHigh_D([" << PARTNAME << "," << PORTNR << "]): Could not add Forced High. Since ROSwrite component has not yet been started." << endlog();
		return;
	}
	...
	for (uint l = 0; l < bodypart_names.size(); l++) {
		if (bodypart_names[l] == PARTNAME) {
			BPID = l + 1;
			log(Info) << "ROSwrite::AddForcedHigh_D([" << PARTNAME << "," << PORTNR << "]): BPID IS: " << BPID << "." << endlog();
		}
	}
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSwrite::AddForcedHigh_D([" << PARTNAME << "," << PORTNR << "]): Could not add Forced High. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}
	
	// Check if the digital out is already added for this BPID and check if a forcedhigh is already added for this BPID, PORTNR.
	if (n_inports_D[BPID-1] <= 0) {
		log(Error) << "ROSwrite::AddForcedHigh_D([" << PARTNAME << "," << PORTNR << "]): Could not add Forced High. There is no digital outs added for this bodypart!" << endlog();
		return;
	}
	if( forcedhigh_status_D[BPID-1][PORTNR-1] ) {
		log(Warning) << "ROSwrite::AddForcedHigh_D: Could not add Forced High. since this already excists for this bodypart." << endlog();
		return;
	}
	
	// Check validity PORTNR
	if( PORTNR <= 0 || PORTNR > n_inports_D[BPID-1]) {
		log(Error) << "ROSwrite::AddForcedHigh_D([" << PARTNAME << "," << PORTNR << "]): Could not add Forced High. Invalid PORTNR: " << PORTNR << ".  1 <= PORTNR <= " << n_inports_D[BPID-1] << "!" << endlog();
		return;
	}
		
	log(Info) << "ROSwrite::AddForcedHigh_D([" << PARTNAME << "," << PORTNR << "]): Added a forced high." << endlog();
	
	// Set status
	forcedhigh_status_D[BPID-1][PORTNR-1] = true;
}
	...
	if (!goodToGO && (aquisition_time - start_time > 10.0)) {
		...
		for(uint l = 0; l < MAX_BODYPARTS; l++) {
			for(uint i = 0; i < n_inports_A[l]; i++) {
				if ( !inports_A[l][i].connected() ) {
					log(Error) << "ROSwrite::CheckAllConnections: Analog inport " << inports_A[l][i].getName() << " is not connected!" << endlog();
				}
			}
			for(uint i = 0; i < n_outports_A[l]; i++) {
				if ( !outports_A[l][i].connected() ) {
					log(Info) << "ROSwrite::CheckAllConnections: Analog outport " << outports_A[l][i].getName() << " is not connected!" << endlog();
				}
			}
		}
		...
		for(uint l = 0; l < MAX_BODYPARTS; l++) {
			for(uint i = 0; i < n_inports_D[l]; i++) {
				if ( !inports_D[l][i].connected() && (!forcedhigh_status_D[l][i])) {
					log(Error) << "ROSwrite::CheckAllConnections: Digital inport " << inports_D[l][i].getName() << " is not connected!" << endlog();
				}
			}
			for(uint i = 0; i < n_outports_D[l]; i++) {
				if ( !outports_D[l][i].connected() ) {
					log(Info) << "ROSwrite::CheckAllConnections: Digital outport " << outports_D[l][i].getName() << " is not connected!" << endlog();
				}
			}
		}
	}
...
void ROSwrite::ReadInputs()
{
	...
	for( uint l = 0; l < MAX_BODYPARTS; l++ ) {
		for( uint i = 0; i < n_inports_D[l]; i++ ) {
			// check if connected (to avoid that old safety values remain true)
...
void ROSwrite::MapInputs2Outputs()
{
		...
		for( uint i = 0; i < n_outports_D[l]; i++ ) {
			for( uint k = 0; k < outport_dimensions_D[l][i]; ++k) {
				if (from_which_inport_D[l][j+k] == 0 || from_which_entry_D[l][j+k] ==0) {
					...
					output_msgs_D[l][i].values[k] = input_D[l][ from_which_inport_D[l][j+k]-1 ];
				}
			}
