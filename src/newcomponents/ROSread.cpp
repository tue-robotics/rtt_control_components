...
using namespace std;
using namespace RTT;
using namespace ROSREAD;

ROSread::ROSread(const string& name) : TaskContext(name, PreOperational)
{
	//! Properties
	...
	//! Operations
	// Add Ins
    addOperation("AddAnalogIns", &ROSread::AddAnalogIns, this, OwnThread)
    ...
    addOperation("AddDigitalIns", &ROSread::AddDigitalIns, this, OwnThread)
		...
		.arg("FROM_WHICH_ENTRY","Array specifying where the input from the inports should go - second specify which entry")
		.arg("PARTNAME","String specifying the name of the part");
    addOperation("AddEncoderIns", &ROSread::AddEncoderIns, this, OwnThread)
	...
	// Math
	addOperation("AddAddition_A", &ROSread::AddAddition_A, this, OwnThread)
	...
	addOperation("AddMultiply_A", &ROSread::AddMultiply_A, this, OwnThread)
	...
	addOperation("AddCompare_A", &ROSread::AddCompare_A, this, OwnThread)
	...
	addOperation("AddTorqueSensor_A", &ROSread::AddTorqueSensor_A, this, OwnThread)
	...
	addOperation("AddFlip_D", &ROSread::AddFlip_D, this, OwnThread)
		.doc("This function will flip the digital value")
		.arg("PARTNAME","Name of the bodypart")
		.arg("PORTNR","Output port number of the digital in");
		
	addOperation("AddEnc2Si_E", &ROSread::AddEnc2Si_E, this, OwnThread)
	...
	addOperation("AddMatrixTransform_E", &ROSread::AddMatrixTransform_E, this, OwnThread)
	...
	addOperation("AddSaturation_E", &ROSread::AddSaturation_E, this, OwnThread)
	...
	addOperation( "ResetEncoders", &ROSread::ResetEncoders, this, OwnThread )
		.doc("Reset an encoder value to a new value")
		.arg("PARTNAME","Name of the bodypart")
		.arg("PORTNR","Output port number of the encoder ins")
		.arg("resetvalues","Values to reset the encoder to");
	
	addOperation( "AddMsgOut_A", &ROSread::AddMsgOut_A, this, OwnThread )
		.doc("Add a std_msg outport with analog msg")
		.arg("PARTNAME","Name of the bodypart")
		.arg("PORTNR","Output port number of the encoder ins");
	addOperation( "AddMsgOut_D", &ROSread::AddMsgOut_D, this, OwnThread )
		.doc("Add a std_msg outport with bool msg")
		.arg("PARTNAME","Name of the bodypart")
		.arg("PORTNR","Output port number of the encoder ins");
	addOperation( "AddMsgOut_E", &ROSread::AddMsgOut_E, this, OwnThread )
...
bool ROSread::startHook()
{
	if (bodypart_names.size() <= 0) {
		log(Error) << "ROSread::startHook: Could not start ROSread. First add a list of the bodyparts that will be added. (at least one) " << bodypart_names.size() << "!" << endlog();
		return false;
	}
...
void ROSread::AddAnalogIns(string PARTNAME, doubles INPORT_DIMENSIONS, doubles OUTPORT_DIMENSIONS, doubles FROM_WHICH_INPORT, doubles FROM_WHICH_ENTRY)
{
	// Init 
	...
	//! Check configuration
	if (!this->isRunning()) {
		log(Error) << "ROSread::AddAnalogIns(" << PARTNAME << "): Could not add AnalogIns. Since ROSread component has not yet been started!" << endlog();
		return;
	}
	// Check if the AnalogIns is already added for this bodypart
	for (uint l = 0; l < MAX_BODYPARTS; l++) {
		if (PARTNAME == added_bodyparts_A[l]) {
			log(Error) << "ROSread::AddAnalogIns(" << PARTNAME << "): Could not add AnalogIns. There is already an AnalogIns for bodypart " << PARTNAME << "!" << endlog();
			return;
		}
	}
	...
	for (uint l = 0; l < bodypart_names.size(); l++) {
		if (bodypart_names[l] == PARTNAME) {
			BPID = l + 1;
			log(Info) << "ROSread::AddAnalogIns(" << PARTNAME << "): BPID IS: " << BPID << "." << endlog();
		}
	}
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSread::AddAnalogIns(" << PARTNAME << "): Could not add AnalogIns. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}

	// Check for invalid number of ports
	if (N_INPORTS < 1 || N_INPORTS > MAX_PORTS) {
		log(Error) << "ROSread::AddAnalogIns(" << PARTNAME << "): Could not add AnalogIns. Invalid number of inports: " << N_INPORTS << "!" << endlog();
		return;
	}    
	if (N_OUTPORTS < 1 || N_OUTPORTS > MAX_PORTS) {
		log(Error) << "ROSread::AddAnalogIns(" << PARTNAME << "): Could not add AnalogIns. Invalid number of outports: " << N_OUTPORTS << "!" << endlog();
		return;
	}
	...
	for(uint i = 0; i < N_INPORTS; i++) {
		if(INPORT_DIMENSIONS[i] < 1 || INPORT_DIMENSIONS[i] > 100) {
			log(Error) << "ROSread::AddAnalogIns(" << PARTNAME << "): Could not add AnalogIns. Invalid inport dimension: " << INPORT_DIMENSIONS[i] << "!" << endlog();
			return;
		}
	...
	for(uint i = 0; i < N_OUTPORTS; i++) {
		if(OUTPORT_DIMENSIONS[i] < 1 || OUTPORT_DIMENSIONS[i] > 100) {
			log(Error) << "ROSread::AddAnalogIns(" << PARTNAME << "): Could not add AnalogIns. Invalid outport dimension: " << OUTPORT_DIMENSIONS[i] << "!" << endlog();
			return;
		}
	...
	if ((FROM_WHICH_INPORT.size() != N_OUTPORT_ENTRIES) || (FROM_WHICH_ENTRY.size() != N_OUTPORT_ENTRIES)) {
		log(Error) << "ROSread::AddAnalogIns(" << PARTNAME << "): Could not add AnalogIns. The number of entries in from_which_inport_A and from_which_entry_A should equal the total number of output values!" << endlog();
		return;
	}
	...
	for(uint k = 0; k < N_OUTPORT_ENTRIES; k++) {
		if( FROM_WHICH_INPORT[k] < 0 || FROM_WHICH_INPORT[k] > N_INPORTS ) {
			log(Error) << "ROSread::AddAnalogIns(" << PARTNAME << "): Could not add AnalogIns. 0 < From_which_inport <= N_INPORTS. -> 0 < " << FROM_WHICH_INPORT[k] << " <= "<< N_INPORTS <<"!" << endlog();
			return;
		}
		else if ( FROM_WHICH_ENTRY[k] < 0 || FROM_WHICH_ENTRY[k] > INPORT_DIMENSIONS[FROM_WHICH_INPORT[k]-1] ) {
			log(Error) << "ROSread::AddAnalogIns(" << PARTNAME << "): Could not add AnalogIns. From_which_entry array contains entry no. " << FROM_WHICH_ENTRY[k] << " which does not exist for inport no. " << FROM_WHICH_INPORT[k] << "!" << endlog();
			return;
		}
	}
	...
	log(Warning) << "ROSread::AddAnalogIns(" << PARTNAME << "): Added AnalogIns with " << N_INPORTS << " inports and " << N_OUTPORTS << " outports." << endlog();

	return;
}

void ROSread::AddDigitalIns(string PARTNAME, doubles INPORT_DIMENSIONS, doubles OUTPORT_DIMENSIONS, doubles FROM_WHICH_INPORT, doubles FROM_WHICH_ENTRY)
{	
	// Init 
	uint N_INPORTS = INPORT_DIMENSIONS.size();
	uint N_OUTPORTS = OUTPORT_DIMENSIONS.size();
	...
	//! Check configuration	
	if (!this->isRunning()) {
		log(Error) << "ROSread::AddDigitalIns(" << PARTNAME << "): Could not add DigitalIns. Since ROSread component has not yet been started!" << endlog();
		return;
	}
	// Check if the bodypart name is in the bodypart_name list and set the BPID accordingly
	for (uint l = 0; l < bodypart_names.size(); l++) {
		if (bodypart_names[l] == PARTNAME) {
			BPID = l + 1;
			log(Info) << "ROSread::AddDigitalIns(" << PARTNAME << "): BPID IS: " << BPID << "." << endlog();
		}
	}
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSread::AddDigitalIns(" << PARTNAME << "): Could not add DigitalIns. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}
	    
	// Check for invalid number of ports
    if(N_INPORTS < 1 || N_INPORTS > MAX_PORTS) {
        log(Error) << "ROSread::AddDigitalIns(" << PARTNAME << "): Could not add DigitalIns. Invalid number of inports: " << N_INPORTS << "!" << endlog();
        return;
    }
    if(N_OUTPORTS < 1 || N_OUTPORTS > MAX_PORTS) {
        log(Error) << "ROSread::AddDigitalIns(" << PARTNAME << "): Could not add DigitalIns. Invalid number of outports: " << N_OUTPORTS << "!" << endlog();
        return;
    }    
	...
	for(uint i = 0; i < N_INPORTS; i++) {
        if(INPORT_DIMENSIONS[i] < 1 || INPORT_DIMENSIONS[i] > 100) {
            log(Error) << "ROSread::AddDigitalIns(" << PARTNAME << "): Could not add DigitalIns. Invalid inport dimension: " << INPORT_DIMENSIONS[i] << "!" << endlog();
            return;
        }
        N_INPORT_ENTRIES += INPORT_DIMENSIONS[i];
    }
    for(uint i = 0; i < N_OUTPORTS; i++) {
        if(OUTPORT_DIMENSIONS[i] != 1 ) {
            log(Error) << "ROSread::AddDigitalIns(" << PARTNAME << "): Could not add DigitalIns. Outport dimension: " << OUTPORT_DIMENSIONS[i] << " should be size 1!" << endlog();
            return;
        }
        N_OUTPORT_ENTRIES += OUTPORT_DIMENSIONS[i];
    }
	...
	// Check if the total number of entries matches the size of FROM_WHICH_INPORT and FROM_WHICH_ENTRY
    if ((FROM_WHICH_INPORT.size() != N_OUTPORT_ENTRIES) || (FROM_WHICH_ENTRY.size() != N_OUTPORT_ENTRIES)) {
        log(Error) << "ROSread::AddDigitalIns(" << PARTNAME << "): Could not add DigitalIns. The number of entries in from_which_inport_D and from_which_entry_D should equal the total number of output values!" << endlog();
        return;
    }
    ...
    for(uint k = 0; k < N_OUTPORT_ENTRIES; k++) {
        if( FROM_WHICH_INPORT[k] <= 0 || FROM_WHICH_INPORT[k] > N_INPORTS ) {
            log(Error) << "ROSread::AddDigitalIns(" << PARTNAME << "): Could not add DigitalIns. 0 < From_which_inport <= N_INPORTS. -> 0 < " << FROM_WHICH_INPORT[k] << " <= "<< N_INPORTS <<"!" << endlog();
            return;
        }
        else if ( FROM_WHICH_ENTRY[k] <= 0 || FROM_WHICH_ENTRY[k] > INPORT_DIMENSIONS[FROM_WHICH_INPORT[k]-1] ) {
            log(Error) << "ROSread::AddDigitalIns(" << PARTNAME << "): Could not add DigitalIns. From_which_entry array contains entry no. " << FROM_WHICH_ENTRY[k] << " which does not exist for inport no. " << FROM_WHICH_INPORT[k] << "!" << endlog();
            return;
        }
    }
    ...
    log(Warning) << "ROSread::AddDigitalIns(" << PARTNAME << "): Added DigitalIns with " << N_INPORTS << " inports and " << N_OUTPORTS << " outports." << endlog();

	return;
}

void ROSread::AddEncoderIns(string PARTNAME, doubles INPORT_DIMENSIONS, doubles OUTPORT_DIMENSIONS, doubles FROM_WHICH_INPORT, doubles FROM_WHICH_ENTRY)
{	
	// Init 
	uint N_INPORTS = INPORT_DIMENSIONS.size();
	uint N_OUTPORTS = OUTPORT_DIMENSIONS.size();
	...
	//! Check configuration	
	if (!this->isRunning()) {
		log(Error) << "ROSread::AddEncoderIns(" << PARTNAME << "): Could not add EncoderIns. Since ROSread component has not yet been started." << endlog();
		return;
	}
	// Check if the EncoderIns is already added for this bodypart
	for(uint l = 0; l < MAX_BODYPARTS; l++) {
		if (PARTNAME == added_bodyparts_E[l]) {
			log(Error) << "ROSread::AddEncoderIns(" << PARTNAME << "): Could not add EncoderIns. There is already an EncoderIns for bodypart " << PARTNAME << "!" << endlog();
			return;
		}
	}
	// Check if the bodypart name is in the bodypart_name list and set the BPID accordingly
	for (uint l = 0; l < bodypart_names.size(); l++) {
		if (bodypart_names[l] == PARTNAME) {
			BPID = l + 1;
			log(Info) << "ROSread::AddEncoderIns(" << PARTNAME << "): BPID IS: " << BPID << "." << endlog();
		}
	}
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSread::AddEncoderIns(" << PARTNAME << "): Could not add EncoderIns. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}	
    
	// Check for invalid number of ports
    if(N_INPORTS < 1 || N_INPORTS > MAX_ENCPORTS) {
        log(Error) << "ROSread::AddEncoderIns(" << PARTNAME << "): Could not add EncoderIns. Invalid number of inports: " << N_INPORTS << "!" << endlog();
        return;
    }
    if(N_OUTPORTS < 1 || N_OUTPORTS > MAX_PORTS) {
        log(Error) << "ROSread::AddEncoderIns(" << PARTNAME << "): Could not add EncoderIns. Invalid number of outports: " << N_OUTPORTS << "!" << endlog();
        return;
    }
    ...
    for(uint i = 0; i < N_INPORTS; i++) {
        if(INPORT_DIMENSIONS[i] != 1) {
            log(Error) << "ROSread::AddEncoderIns(" << PARTNAME << "): Could not add EncoderIns. Inport_dimensions cannot contain value other than one!" << endlog();
            return;
        }
        N_INPORT_ENTRIES += INPORT_DIMENSIONS[i];
    }
    ...
    if(OUTPORT_DIMENSIONS.size() != 1) {
		log(Warning) << "ROSread::AddEncoderIns(" << PARTNAME << "): Outport_dimensions is advised to be of size 1!" << endlog();
	}
    for(uint i = 0; i < N_OUTPORTS; i++) {
        if(OUTPORT_DIMENSIONS[i] < 1) {
            log(Error) << "ROSread::AddEncoderIns(" << PARTNAME << "): Could not add EncoderIns. Outport_dimensions cannot contain value smaller than one!" << endlog();
            return;
        }
        N_OUTPORT_ENTRIES += OUTPORT_DIMENSIONS[i];
    }
	...
	// Check if the total number of entries matches the size of FROM_WHICH_INPORT and FROM_WHICH_ENTRY
    if ((FROM_WHICH_INPORT.size() != N_OUTPORT_ENTRIES) || (FROM_WHICH_ENTRY.size() != N_OUTPORT_ENTRIES)) {
        log(Error) << "ROSread::AddEncoderIns(" << PARTNAME << "): Could not add EncoderIns. The number of entries in from_which_inport_E and from_which_entry_E should equal the total number of output values." << endlog();
        return;
    }
    ...
    for(uint k = 0; k < N_OUTPORT_ENTRIES; k++) {
        if( FROM_WHICH_INPORT[k] <= 0 || FROM_WHICH_INPORT[k] > N_INPORTS ) {
            log(Error) << "ROSread::AddEncoderIns(" << PARTNAME << "): Could not add EncoderIns. 0 < From_which_inport <= N_INPORTS. -> 0 < " << FROM_WHICH_INPORT[k] << " <= "<< N_INPORTS <<"!" << endlog();
            return;
        }
        else if ( FROM_WHICH_ENTRY[k] != 1 ) {
            log(Error) << "ROSread::AddEncoderIns(" << PARTNAME << "): Could not add EncoderIns. From_which_entry array contains entry no. " << FROM_WHICH_ENTRY[k] << " which should be 1 for a encoder in.!" << endlog();
            return;
        }
    }
	...
	log(Warning) << "ROSread::AddEncoderIns(" << PARTNAME << "): Added EncoderIns with " << N_INPORTS << " inports and " << N_OUTPORTS << " outports." << endlog();
...
// Analog
void ROSread::AddAddition_A(string PARTNAME, int PORTNR, doubles ADDVALUES)
{
	// init
	uint BPID;
	...
	//! Check configuration	
	if (!this->isRunning()) {
		log(Error) << "ROSread::AddAddition_A([" << PARTNAME << "," << PORTNR << "]): Could not add Addition_A. Since ROSread component has not yet been started." << endlog();
		return;
	}
	...
	for (uint l = 0; l < bodypart_names.size(); l++) {
		if (bodypart_names[l] == PARTNAME) {
			BPID = l + 1;
			log(Info) << "ROSread::AddAddition_A([" << PARTNAME << "," << PORTNR << "]): BPID IS: " << BPID << "." << endlog();
		}
	}
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSread::AddAddition_A([" << PARTNAME << "," << PORTNR << "]): Could not add addition. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}
	
	if( PORTNR <= 0 || PORTNR > n_outports_A[BPID-1]) {
		log(Error) << "ROSread::AddAddition_A([" << PARTNAME << "," << PORTNR << "]): Could not add addition. Invalid PORTNR: " << PORTNR << ".  1 <= PORTNR <= " << n_outports_A[BPID-1] << "!" << endlog();
		return;
	}
	if( ADDVALUES.size() != outport_dimensions_A[BPID-1][PORTNR-1] ) {
		log(Error) << "ROSread::AddAddition_A([" << PARTNAME << "," << PORTNR << "]): Could not add addition. Invalid size of ADDVALUES " << ADDVALUES.size() << ". Should have been of size :" << outport_dimensions_A[BPID-1][PORTNR-1] << "!" << endlog();
		return;
	}
	if( multiply_status_A[BPID-1][PORTNR-1] ) {
		log(Error) << "ROSread::AddAddition_A([" << PARTNAME << "," << PORTNR << "]): Could not add addition. For this output a multiplier is already there" << endlog();
	...
	if( addition_status_A[BPID-1][PORTNR-1] ) {
		log(Warning) << "ROSread::AddAddition_A([" << PARTNAME << "," << PORTNR << "]): Overwritten existing addition." << endlog();
	} else {
		log(Info) << "ROSread::AddAddition_A([" << PARTNAME << "," << PORTNR << "]): Added addition." << endlog();
	}
...
void ROSread::AddMultiply_A(string PARTNAME, int PORTNR, doubles MULTIPLYFACTOR)
{
	// init
	uint BPID;	
	...
	//! Check configuration	
	if (!this->isRunning()) {
		log(Error) << "ROSread::AddMultiply_A([" << PARTNAME << "," << PORTNR << "]): Could not add Multiply_A. Since ROSread component has not yet been started." << endlog();
		return;
	}
	// Check if the bodypart name is in the bodypart_name list and set the BPID accordingly
	for (uint l = 0; l < bodypart_names.size(); l++) {
		if (bodypart_names[l] == PARTNAME) {
			BPID = l + 1;
			log(Info) << "ROSread::AddMultiply_A([" << PARTNAME << "," << PORTNR << "]): BPID IS: " << BPID << "." << endlog();
		}
	}
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSread::AddMultiply_A([" << PARTNAME << "," << PORTNR << "]): Could not add multiply. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}
	
	if( PORTNR <= 0 || PORTNR > n_outports_A[BPID-1]) {
		log(Error) << "ROSread::AddMultiply_A([" << PARTNAME << "," << PORTNR << "]): Could not add multiply. Invalid PORTNR: " << PORTNR << ".  1 <= PORTNR <= " << n_outports_A[BPID-1] << "!" << endlog();
		return;
	}
	if( MULTIPLYFACTOR.size() != outport_dimensions_A[BPID-1][PORTNR-1] ) {
		log(Error) << "ROSread::AddMultiply_A([" << PARTNAME << "," << PORTNR << "]): Could not add multiplier. Invalid size of MULTIPLYFACTOR " << MULTIPLYFACTOR.size() << ". Should have be of size :" << outport_dimensions_A[BPID-1][PORTNR-1] << "!" << endlog();
		return;
	}	
	...
	if( multiply_status_A[BPID-1][PORTNR-1] ) {
		log(Warning) << "ROSread::AddMultiply_A([" << PARTNAME << "," << PORTNR << "]): Overwritten existing multiplier." << endlog();
	} else {
		log(Warning) << "ROSread::AddMultiply_A([" << PARTNAME << "," << PORTNR << "]): Added multiplier." << endlog();
	}
...
void ROSread::AddCompare_A(string PARTNAME, int PORTNR, string COMPARISON, doubles VALUES)
{
	// init
	uint BPID;
	...
	//! Check configuration	
	if (!this->isRunning()) {
		log(Error) << "ROSread::AddCompare_A([" << PARTNAME << "," << PORTNR << "]): Could not add comparison. Since ROSread component has not yet been started." << endlog();
		return;
	}
	// Check if the bodypart name is in the bodypart_name list and set the BPID accordingly
	for (uint l = 0; l < bodypart_names.size(); l++) {
		if (bodypart_names[l] == PARTNAME) {
			BPID = l + 1;
			log(Info) << "ROSread::AddCompare_A([" << PARTNAME << "," << PORTNR << "]): BPID IS: " << BPID << "." << endlog();
		}
	}
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSread::AddCompare_A([" << PARTNAME << "," << PORTNR << "]): Could not add comparison. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}
	 
	if( PORTNR <= 0 || PORTNR > n_outports_A[BPID-1]) {
		log(Error) << "ROSread::AddCompare_A([" << PARTNAME << "," << PORTNR << "]): Could not add comparison. Invalid PORTNR: " << PORTNR << ".  1 <= PORTNR <= " << n_outports_A[BPID-1] << "!" << endlog();
		return;
	}
	if (VALUES.size() != outport_dimensions_A[BPID-1][PORTNR-1]) {
		log(Error) << "ROSread::AddCompare_A([" << PARTNAME << "," << PORTNR << "]): Could not add comparison. VALUES.size(): " << VALUES.size() << " should be equal to outport_dimensions_A[BPID-1][PORTNR-1]:!" << outport_dimensions_A[BPID-1][PORTNR-1] <<"." << endlog();
		return;
	}
	if (COMPARISON != "<" && COMPARISON != "<=" && COMPARISON != "==" && COMPARISON != "=>" && COMPARISON != ">") {
		log(Error) << "ROSread::AddCompare_A([" << PARTNAME << "," << PORTNR << "]): Could not add comparison. COMPARISON: " << COMPARISON << " should be one of these ['<','<=','==','=>','>']" << endlog();
		return;
	}
	...
	if( comparison_status_A[BPID-1][PORTNR-1] ) {
		log(Warning) << "ROSread::AddCompare_A([" << PARTNAME << "," << PORTNR << "]): Overwritten existing comparison." << endlog();
	} else {
		log(Info) << "ROSread::AddCompare_A([" << PARTNAME << "," << PORTNR << "]): Added a comparison." << endlog();
	}
...
void ROSread::AddTorqueSensor_A(string PARTNAME, int PORTNR, doubles COEFFICIENT1, doubles COEFFICIENT2, doubles COEFFICIENT3)
{
	// init
	uint BPID;
	...
	//! Check configuration	
	if (!this->isRunning()) {
		log(Error) << "ROSread::AddCompare_A([" << PARTNAME << "," << PORTNR << "]): Could not add TorqueSensor. Since ROSread component has not yet been started." << endlog();
		return;
	}
	// Check if the bodypart name is in the bodypart_name list and set the BPID accordingly
	for (uint l = 0; l < bodypart_names.size(); l++) {
	...
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSread::AddCompare_A([" << PARTNAME << "," << PORTNR << "]): Could not add TorqueSensor. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}
	 
	if( PORTNR <= 0 || PORTNR > n_outports_A[BPID-1]) {
		log(Error) << "ROSread::AddCompare_A([" << PARTNAME << "," << PORTNR << "]): Could not add TorqueSensor. Invalid PORTNR: " << PORTNR << ".  1 <= PORTNR <= " << n_outports_A[BPID-1] << "!" << endlog();
		return;
	}
	if (COEFFICIENT1.size() != outport_dimensions_A[BPID-1][PORTNR-1] || COEFFICIENT2.size() != outport_dimensions_A[BPID-1][PORTNR-1] || COEFFICIENT3.size() != outport_dimensions_A[BPID-1][PORTNR-1]) {
		log(Error) << "ROSread::AddCompare_A([" << PARTNAME << "," << PORTNR << "]): Could not add TorqueSensor. COEFFICIENT1.size(): " << COEFFICIENT1.size() << ", COEFFICIENT2.size(): " << COEFFICIENT1.size() << " and COEFFICIENT3.size(): " << COEFFICIENT3.size() << " should be equal to outport_dimensions_A[BPID-1][PORTNR-1]:!" << outport_dimensions_A[BPID-1][PORTNR-1] <<"." << endlog();
		return;
	}
	...
	if( torquesensor_status_A[BPID-1][PORTNR-1] ) {
		log(Warning) << "ROSread::AddCompare_A([" << PARTNAME << "," << PORTNR << "]): Overwritten existing TorqueSensor." << endlog();
	} else {
		log(Warning) << "ROSread::AddCompare_A([" << PARTNAME << "," << PORTNR << "]): Added a TorqueSensor." << endlog();
	}
...
// Digital
void ROSread::AddFlip_D(string PARTNAME, int PORTNR)
{
	// init
	uint BPID;	
	...
	//! Check configuration	
	if (!this->isRunning()) {
		log(Error) << "ROSread::AddFlip_D([" << PARTNAME << "," << PORTNR << "]): Could not add Flip_D. Since ROSread component has not yet been started." << endlog();
		return;
	}
	// Check if the bodypart name is in the bodypart_name list and set the BPID accordingly
	for (uint l = 0; l < bodypart_names.size(); l++) {
		if (bodypart_names[l] == PARTNAME) {
			BPID = l + 1;
			log(Info) << "ROSread::AddFlip_D([" << PARTNAME << "," << PORTNR << "]): BPID IS: " << BPID << "." << endlog();
		}
	}
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSread::AddFlip_D([" << PARTNAME << "," << PORTNR << "]): Could not add flip. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}
	
	if( PORTNR <= 0 || PORTNR > n_outports_D[BPID-1]) {
		log(Error) << "ROSread::AddFlip_D([" << PARTNAME << "," << PORTNR << "]): Could not add flip. Invalid PORTNR: " << PORTNR << ".  1 <= PORTNR <= " << n_outports_D[BPID-1] << "!" << endlog();
		return;
	}
	
	if( flip_status_D[BPID-1][PORTNR-1] ) {
		log(Warning) << "ROSread::AddFlip_D([" << PARTNAME << "," << PORTNR << "]): Overwritten existing flip." << endlog();
	} else {
		log(Info) << "ROSread::AddFlip_D([" << PARTNAME << "," << PORTNR << "]): Added a flip." << endlog();
	}
...
// Encoder
void ROSread::AddEnc2Si_E(string PARTNAME, int PORTNR, doubles ENCODERBITS, doubles ENC2SI)
{
	// init
	uint BPID;	
	...
	//! Check configuration	
	if (!this->isRunning()) {
		log(Error) << "ROSread::AddEnc2Si_E([" << PARTNAME << "," << PORTNR << "]): Could not add Enc2Si_E. Since ROSread component has not yet been started." << endlog();
		return;
	}
	// Check if the bodypart name is in the bodypart_name list and set the BPID accordingly
	for (uint l = 0; l < bodypart_names.size(); l++) {
		if (bodypart_names[l] == PARTNAME) {
			BPID = l + 1;
			log(Info) << "ROSread::AddEnc2Si_E([" << PARTNAME << "," << PORTNR << "]): BPID IS: " << BPID << "." << endlog();
		}
	}
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSread::AddEnc2Si_E([" << PARTNAME << "," << PORTNR << "]): Could not add enc2si. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}
	
	if( PORTNR <= 0 || PORTNR > n_outports_E[BPID-1]) {
		log(Error) << "ROSread::AddEnc2Si_E([" << PARTNAME << "," << PORTNR << "]): Could not add enc2si. Invalid PORTNR: " << PORTNR << ".  1 <= PORTNR <= " << n_outports_E[BPID-1] << "!" << endlog();
		return;
	}
	if( ENCODERBITS.size() != outport_dimensions_E[BPID-1][PORTNR-1] || ENC2SI.size() != outport_dimensions_E[BPID-1][PORTNR-1] ) {
		log(Error) << "ROSread::AddEnc2Si_E([" << PARTNAME << "," << PORTNR << "]): Could not add enc2si. Invalid size of ENC2SI. Should have be of size :" << outport_dimensions_E[BPID-1][PORTNR-1] << "!" << endlog();
		return;
	}
	for(uint k = 0; k < ENCODERBITS.size(); k++) {
		if(ENC2SI[k] > 1.0 ) {
			log(Error) << "ROSread::AddEnc2Si_E([" << PARTNAME << "," << PORTNR << "]): Could not add enc2si. Currently Enc2SI values > 1 are not supported. Your " << k <<"th enc2si was: " << ENC2SI[k] << "!" << endlog();
			return;
		}
	}
	...
	if( enc2si_status_E[BPID-1][PORTNR-1] ) {
		log(Warning) << "ROSread::AddEnc2Si_E([" << PARTNAME << "," << PORTNR << "]): Overwritten existing enc2si." << endlog();
	} else {
		log(Info) << "ROSread::AddEnc2Si_E([" << PARTNAME << "," << PORTNR << "]): Added a enc2si." << endlog();
	}
...
void ROSread::AddMatrixTransform_E(string PARTNAME, int PORTNR, double INPUTSIZE, double OUTPUTSIZE)
{
	...
	//! Check configuration	
	if (!this->isRunning()) {
		log(Error) << "ROSread::AddMatrixTransform_E([" << PARTNAME << "," << PORTNR << "]): Could not add MatrixTransform_E. Since ROSread component has not yet been started." << endlog();
		return;
	}
	// Check if the bodypart name is in the bodypart_name list and set the BPID accordingly
	for (uint l = 0; l < bodypart_names.size(); l++) {
		if (bodypart_names[l] == PARTNAME) {
			BPID = l + 1;
			log(Info) << "ROSread::AddMatrixTransform_E([" << PARTNAME << "," << PORTNR << "]): BPID IS: " << BPID << "." << endlog();
		}
	}
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSread::AddMatrixMultiplication_E([" << PARTNAME << "," << PORTNR << "]): Could not add matrix transform. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}
	 
	if( PORTNR <= 0 || PORTNR > n_outports_E[BPID-1]) {
		log(Error) << "ROSread::AddMatrixMultiplication_E([" << PARTNAME << "," << PORTNR << "]): Could not add matrix transform. Invalid PORTNR: " << PORTNR << ".  1 <= PORTNR <= " << n_outports_E[BPID-1] << "!" << endlog();
		return;
	}
	if( matrixtransform_status_E[BPID-1][PORTNR-1] ) {
		log(Warning) << "ROSread::AddMatrixTransform_E([" << PARTNAME << "," << PORTNR << "]): Could not add Matrix Transform, since this already excists for this bodypart. Overwriting is not supported at the moment" << endlog();
		return;
	}
	if( OUTPUTSIZE != outport_dimensions_E[BPID-1][PORTNR-1]) {
		if( OUTPUTSIZE < outport_dimensions_E[BPID-1][PORTNR-1]) { 	// smaller matrix then nr of inputs is allowed, not recommended
			log(Warning) << "ROSread::AddMatrixTransform_E([" << PARTNAME << "," << PORTNR << "]): INPUTSIZE: " << OUTPUTSIZE << " is smafller than the size of the outport outport_dimensions_E[BPID-1][PORTNR-1]:" << outport_dimensions_E[BPID-1][PORTNR-1] << "!" << endlog();
		} else { 										// larger matrix then nr of inputs is not allowed
			log(Error) << "ROSread::AddMatrixTransform_E([" << PARTNAME << "," << PORTNR << "]): Could not add matrix transform. INPUTSIZE: " << OUTPUTSIZE << " is larger than the size of the outport outport_dimensions_E[BPID-1][PORTNR-1]:" << inport_dimensions_E[BPID-1][PORTNR-1] << "!" << endlog();
			return;
		}
	}
	...
	log(Info) << "ROSread::AddMatrixTransform_E([" << PARTNAME << "," << PORTNR << "]): Added a matrix transform." << endlog();
	
	// Set status
	matrixtransform_status_E[BPID-1][PORTNR-1] = true;
}

void ROSread::AddSaturation_E(string PARTNAME, int PORTNR, doubles SATURATIONMIN, doubles SATURATIONMAX)
{
	// init
	uint BPID;
	...
	//! Check configuration	
	if (!this->isRunning()) {
		log(Error) << "ROSread::AddSaturation_E([" << PARTNAME << "," << PORTNR << "]): Could not add saturation. Since ROSread component has not yet been started." << endlog();
		return;
	}
	// Check if the bodypart name is in the bodypart_name list and set the BPID accordingly
	for (uint l = 0; l < bodypart_names.size(); l++) {
		if (bodypart_names[l] == PARTNAME) {
			BPID = l + 1;
			log(Info) << "ROSread::AddSaturation_E([" << PARTNAME << "," << PORTNR << "]): BPID IS: " << BPID << "." << endlog();
		}
	}
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSread::AddSaturation_E([" << PARTNAME << "," << PORTNR << "]): Could not add saturation. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}
	 
	if( PORTNR <= 0 || PORTNR > n_outports_E[BPID-1]) {
		log(Error) << "ROSread::AddSaturation_E([" << PARTNAME << "," << PORTNR << "]): Could not add saturation. Invalid PORTNR: " << PORTNR << ".  1 <= PORTNR <= " << n_outports_E[BPID-1] << "!" << endlog();
		return;
	}
	if (SATURATIONMIN.size() != outport_dimensions_E[BPID-1][PORTNR-1] || SATURATIONMAX.size() != outport_dimensions_E[BPID-1][PORTNR-1]) {
		log(Error) << "ROSread::AddSaturation_E([" << PARTNAME << "," << PORTNR << "]): Could not add saturation. SATURATIONMIN.size(): " << SATURATIONMIN.size() << " and SATURATIONMAX.size():" << SATURATIONMAX.size() << " should be equal to outport_dimensions_E[BPID-1][PORTNR-1]:!" << outport_dimensions_E[BPID-1][PORTNR-1] <<"." << endlog();
		return;
	}
	for(uint k = 0; k < SATURATIONMIN.size(); k++) {
		if (SATURATIONMIN[k] > SATURATIONMAX[k]) {
			log(Error) << "ROSread::AddSaturation_E([" << PARTNAME << "," << PORTNR << "]): Could not add saturation. SATURATIONMIN[" << k << "]: " << SATURATIONMIN[k] << " can't be larger then SATURATIONMAX[" << k << "]: " << SATURATIONMAX[k] << "!" << endlog();
			return;
		}
	}
	...
	if( saturation_status_E[BPID-1][PORTNR-1] ) {
		log(Warning) << "ROSread::AddSaturation_E([" << PARTNAME << "," << PORTNR << "]): Overwritten existing saturation." << endlog();
	} else {
		log(Warning) << "ROSread::AddSaturation_E([" << PARTNAME << "," << PORTNR << "]): Added a saturation." << endlog();
	}
...
void ROSread::AddMsgOut_A(string PARTNAME, int PORTNR)
{
	// init
	uint BPID;
	...
	//! Check configuration	
	if (!this->isRunning()) {
		log(Error) << "ROSread::AddMsgOut_A([" << PARTNAME << "," << PORTNR << "]): Could not add MsgOut_A. Since ROSread component has not yet been started." << endlog();
		return;
	}
	// Check if the bodypart name is in the bodypart_name list and set the BPID accordingly
	for (uint l = 0; l < bodypart_names.size(); l++) {
		if (bodypart_names[l] == PARTNAME) {
			BPID = l + 1;
			log(Info) << "ROSread::AddMsgOut_A([" << PARTNAME << "," << PORTNR << "]): BPID IS: " << BPID << "." << endlog();
		}
	}
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSread::AddMsgOut_A([" << PARTNAME << "," << PORTNR << "]): Could not add MsgOut_A. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}
	...
	log(Info) << "ROSread::AddMsgOut_A([" << PARTNAME << "," << PORTNR << "]): Added MsgOut_A." << endlog();
}

void ROSread::AddMsgOut_D(string PARTNAME, int PORTNR)
{
	// init
	uint BPID;
	...
	//! Check configuration	
	if (!this->isRunning()) {
		log(Error) << "ROSread::AddMsgOut_D([" << PARTNAME << "," << PORTNR << "]): Could not add MsgOut_D. Since ROSread component has not yet been started." << endlog();
		return;
	}
	// Check if the bodypart name is in the bodypart_name list and set the BPID accordingly
	for (uint l = 0; l < bodypart_names.size(); l++) {
		if (bodypart_names[l] == PARTNAME) {
			BPID = l + 1;
			log(Info) << "ROSread::AddMsgOut_D([" << PARTNAME << "," << PORTNR << "]): BPID IS: " << BPID << "." << endlog();
		}
	}
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSread::AddMsgOut_D([" << PARTNAME << "," << PORTNR << "]): Could not add MsgOut_D. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}
	...
	log(Info) << "ROSread::AddMsgOut_D([" << PARTNAME << "," << PORTNR << "]): Added MsgOut_D." << endlog();
}

void ROSread::AddMsgOut_E(string PARTNAME, int PORTNR, strings JOINT_NAMES)
{	
	// init
	uint BPID;
	...
	//! Check configuration	
	if (!this->isRunning()) {
		log(Error) << "ROSread::AddMsgOut_E([" << PARTNAME << "," << PORTNR << "]): Could not add MsgOut_E. Since ROSread component has not yet been started." << endlog();
		return;
	}
	...
	for (uint l = 0; l < bodypart_names.size(); l++) {
		if (bodypart_names[l] == PARTNAME) {
			BPID = l + 1;
			log(Info) << "ROSread::AddMsgOut_E([" << PARTNAME << "," << PORTNR << "]): BPID IS: " << BPID << "." << endlog();
		}
	}
		
	if ( 0 >= BPID || BPID > bodypart_names.size()) {
		log(Error) << "ROSread::AddMsgOut_E([" << PARTNAME << "," << PORTNR << "]): Could not add MsgOut_E. Invalid BPID. Should satisfy  0 < BPID <= bodypart_names.size(). -> 0 < " << BPID << " <= " << bodypart_names.size() << "!" << endlog(); 
		return;
	}
...
//! Functions to edit inputs
void ROSread::CheckAllConnections()
{
		...
		for(uint l = 0; l < MAX_BODYPARTS; l++) {		
			for(uint i = 0; i < n_inports_A[l]; i++) {
				if ( !inports_A[l][i].connected() ) {
					log(Error) << "ROSread::CheckAllConnections: Analog inport " << inports_A[l][i].getName() << " is not connected!" << endlog();
				}
			}
			for(uint i = 0; i < n_outports_A[l]; i++) {
				if ( !outports_A[l][i].connected() ) {
					log(Info) << "ROSread::CheckAllConnections: Analog outport " << outports_A[l][i].getName() << " is not connected!" << endlog();
				}
			}
		}
		...
		for(uint l = 0; l < MAX_BODYPARTS; l++) {		
			for(uint i = 0; i < n_inports_D[l]; i++) {
				if ( !inports_D[l][i].connected() ) {
					log(Error) << "ROSread::CheckAllConnections: Digital inport " << inports_D[l][i].getName() << " is not connected!" << endlog();
				}
			}
			for(uint i = 0; i < n_outports_D[l]; i++) {
				if ( !outports_D[l][i].connected() ) {
					log(Info) << "ROSread::CheckAllConnections: Digital outport " << outports_D[l][i].getName() << " is not connected!" << endlog();
				}
			}
		}
		...
		for(uint l = 0; l < MAX_BODYPARTS; l++) {	
			for(uint i = 0; i < n_inports_E[l]; i++) {
				if ( !inports_E[l][i].connected() ) {
					log(Error) << "ROSread::CheckAllConnections: Encoder inport " << inports_E[l][i].getName() << " is not connected!" << endlog();
				}
			}
			for(uint i = 0; i < n_outports_E[l]; i++) {
				if ( !outports_E[l][i].connected() ) {
					log(Info) << "ROSread::CheckAllConnections: Encoder outport " << outports_E[l][i].getName() << " is not connected!" << endlog();
				}
				if ( !outports_E_vel[l][i].connected() ) {
					log(Info) << "ROSread::CheckAllConnections: Encoder velocity outport " << outports_E_vel[l][i].getName() << " is not connected!" << endlog();
				}
			}
		}
	}
...
void ROSread::MapInputs2Outputs()
{
	// Analog
	for( uint l = 0; l < MAX_BODYPARTS; l++ ) {
		uint j = 0;
	...
	for( uint l = 0; l < MAX_BODYPARTS; l++ ) {
		...
		for( uint i = 0; i < n_outports_E[l]; i++ ) {
			for( uint k = 0; k < outport_dimensions_E[l][i]; ++k) {
				//log(Warning) << "ROSread::MapInputs2Outputs: intermediate_E[l"<< l << "][i"<< i << "][k"<< k << "][j+k"<< j+k << "] = (double) input_msgs_E[l"<< l << "][ " << from_which_inport_E[l][j+k]-1 << "], " << input_msgs_E[l][from_which_inport_E[l][j+k]-1].value << "!" << endlog();
			...
			if (comparison_status_A[l][i]) {
				for ( uint k = 0; k < outport_dimensions_A[l][i]; k++ ) {
					...
					if (comparison_types_A[l][i] == "<") {
						...
						log(Error) << "ROSread::Calculate_A:Comparison(" << bodypart_names[l] << ", port " << i+1 << "): Invalid comparison type: " << comparison_types_A[l][i] << ", should be one of ['<','<=','==','=>','>']s" << endlog();
					}
				}
			}
		}
...
void ROSread::ResetEncoders(int BPID, int PORTNR, doubles RESETVALUES )
{
	if( BPID <= 0 || BPID > bodypart_names.size()) {
		log(Error) << "ROSread::ResetEncoders(" << BPID << bodypart_names[BPID-1] << ", port " << PORTNR << "): Could not reset encoders. Invalid BPID: " << BPID << ".  1 <= BPID <= " << bodypart_names.size() << "!" << endlog();
		return;
	}
	if( PORTNR <= 0 || PORTNR > n_outports_E[BPID-1]) {
		log(Error) << "ROSread::ResetEncoders(" << BPID << bodypart_names[BPID-1] << ", port " << PORTNR << "): Could not reset encoders. Invalid PORTNR: " << PORTNR << ".  1 <= PORTNR <= " << n_outports_E[BPID-1] << "!" << endlog();
		return;
	}
	if (!enc2si_status_E[BPID-1][PORTNR-1]) {
		log(Error)<<"ROSread::ResetEncoders(" << BPID << bodypart_names[BPID-1] << ", port " << PORTNR << "): Could not reset encoders. The function AddEnc2Si has not yet been called for bodypart with BPID : " << BPID <<"."<<endlog();
		return;		
	}
	if (RESETVALUES.size() != outport_dimensions_E[BPID-1][PORTNR-1]) {
		log(Error)<<"ROSread::ResetEncoders(" << BPID << bodypart_names[BPID-1] << ", port " << PORTNR << "): Could not reset encoders. Received incorrect sized initialize signal: " << RESETVALUES.size() << ". Should be of size" << outport_dimensions_E[BPID-1][PORTNR-1] <<"."<<endlog();
		return;
	}
	...
	for ( uint k = 0; k < RESETVALUES.size(); k++ ) {
		...
		//log(Warning)<<"ROSread::ResetEncoders(" << BPID << bodypart_names[BPID-1] << ", port " << PORTNR << "): Resetting Encoders. initpos_E[BPID-1][PORTNR-1][0] = output_E[BPID-1][PORTNR-1][0] - RESETVALUES[0] -> : " << initpos_E[BPID-1][PORTNR-1][0] << " = " << output_E[BPID-1][PORTNR-1][0]  << " - " << RESETVALUES[0]  << "!"<<endlog();
	}
}

ORO_CREATE_COMPONENT(ROSREAD::ROSread)
