...
void ADIns::updateHook()
{
    ...
    for( uint i = 0; i < n_outports_E; i++ ) {
        for( uint k = 0; k < outport_dimensions_E[i]; ++k) {
            output_E[i][k] = input_msgs_E[ from_which_inport_E[j]-1 ].values[ from_which_entry_E[j]-1 ];
            ++j;
        }
...
void ADIns::AddAnalogIns(doubles INPORT_DIMENSIONS, doubles OUTPORT_DIMENSIONS, doubles FROM_WHICH_INPORT, doubles FROM_WHICH_ENTRY)
{
    ...
    for(uint j = 0; j < N_OUTPORT_ENTRIES; ++j) {
        if( FROM_WHICH_INPORT[j] > (n_inports_A+N_INPORTS) || FROM_WHICH_INPORT[j] <= 0 ) {
            log(Error) << "ADIns::AddAnalogIns: Could not add AnalogIn. From_which_inport array contains port no. " << FROM_WHICH_INPORT[j] << " which does not exist according to inport_dimensions_A!" << endlog();
            return;
        }
        else if ( FROM_WHICH_ENTRY[j] > INPORT_DIMENSIONS[ FROM_WHICH_INPORT[j]-1-n_inports_A] || FROM_WHICH_ENTRY[j] <= 0 ) {
            log(Error) << "ADIns::AddAnalogIns: Could not add AnalogIn. From_which_entry array contains entry no. " << FROM_WHICH_ENTRY[j] << " which does not exist for inport no. " << FROM_WHICH_INPORT[j] << "!" << endlog();
            return;
        }
    }
...
void ADIns::AddDigitalIns(doubles INPORT_DIMENSIONS, doubles OUTPORT_DIMENSIONS, doubles FROM_WHICH_INPORT, doubles FROM_WHICH_ENTRY)
{
	// Init 
	uint N_INPORTS = INPORT_DIMENSIONS.size();
	uint N_OUTPORTS = OUTPORT_DIMENSIONS.size();
	...
	// Check for invalid number of ports
    if(N_INPORTS < 1 || N_INPORTS > MAX_PORTS) {
        log(Error) << "ADIns::AddDigitalIns: Could not add DigitalIn. Invalid number of inports: " << N_INPORTS << "!" << endlog();
        return;
    }
    if(N_OUTPORTS < 1 || N_OUTPORTS > MAX_PORTS) {
        log(Error) << "ADIns::AddDigitalIns: Could not add DigitalIn. Invalid number of outports: " << N_OUTPORTS << "!" << endlog();
        return;
    }
    ...
    for(uint i = 0; i < N_INPORTS; i++) {
        if(INPORT_DIMENSIONS[i] < 1) {
            log(Error) << "ADIns::AddDigitalIns: Could not add DigitalIn. Inport_dimensions cannot contain value smaller than one!" << endlog();
            return;
        }
        N_INPORT_ENTRIES += INPORT_DIMENSIONS[i];
    }
    for(uint i = 0; i < N_OUTPORTS; i++) {
        if(OUTPORT_DIMENSIONS[i] < 1) {
            log(Error) << "ADIns::AddDigitalIns: Could not add DigitalIn. Outport_dimensions cannot contain value smaller than one!" << endlog();
            return;
        }
        N_OUTPORT_ENTRIES += OUTPORT_DIMENSIONS[i];
    }
	...
	// Check if the total number of entries matches the size of FROM_WHICH_INPORT and FROM_WHICH_ENTRY
    if ((FROM_WHICH_INPORT.size() != N_OUTPORT_ENTRIES) || (FROM_WHICH_ENTRY.size() != N_OUTPORT_ENTRIES)) {
        log(Error) << "ADIns::AddDigitalIns: Could not add DigitalIn. The number of entries in from_which_inport_D and from_which_entry_D should equal the total number of output values." << endlog();
        return;
    }
    ...
    for(uint j = 0; j < N_OUTPORT_ENTRIES; j++) {
        if( FROM_WHICH_INPORT[j] > (n_inports_D+N_INPORTS) || FROM_WHICH_INPORT[j] <= 0 ) {
            log(Error) << "ADIns::AddDigitalIns: Could not add DigitalIn. From_which_inport array contains port no. " << FROM_WHICH_INPORT[j] << " which does not exist according to inport_dimensions_D!" << endlog();
            return;
        }
        else if ( FROM_WHICH_ENTRY[j] > INPORT_DIMENSIONS[ FROM_WHICH_INPORT[j]-1-n_inports_D] || FROM_WHICH_ENTRY[j] <= 0 ) {
            log(Error) << "ADIns::AddDigitalIns: Could not add DigitalIn. From_which_entry array contains entry no. " << FROM_WHICH_ENTRY[j] << " which does not exist for inport no. " << FROM_WHICH_INPORT[j] << "!" << endlog();
            return;
        }
    }
    ...
    for( uint i = n_inports_D; i < (n_inports_D+N_INPORTS); i++ ) {
		addEventPort( "Din"+to_string(i+1), inports_D[i] );
	}
    ...
    log(Warning) << "ADIns::AddDigitalIns: Succesfully added DigitalIns with " << N_INPORTS << " inports and " << N_OUTPORTS << " outports!" << endlog();
    
	return;
}

void ADIns::AddEncoderIns(doubles INPORT_DIMENSIONS, doubles OUTPORT_DIMENSIONS, doubles FROM_WHICH_INPORT, doubles FROM_WHICH_ENTRY)
{
	// Init 
	uint N_INPORTS = INPORT_DIMENSIONS.size();
	uint N_OUTPORTS = OUTPORT_DIMENSIONS.size();
	...
	// Check for invalid number of ports
    if(N_INPORTS < 1 || N_INPORTS > MAX_PORTS) {
        log(Error) << "ADIns::AddEncoderIns: Could not add EncoderIn. Invalid number of inports: " << N_INPORTS << "!" << endlog();
        return;
    }
    if(N_OUTPORTS < 1 || N_OUTPORTS > MAX_PORTS) {
        log(Error) << "ADIns::AddEncoderIns: Could not add EncoderIn. Invalid number of outports: " << N_OUTPORTS << "!" << endlog();
        return;
    }
    ...
    for(uint i = 0; i < N_INPORTS; i++) {
        if(INPORT_DIMENSIONS[i] < 1) {
            log(Error) << "ADIns::AddEncoderIns: Could not add EncoderIn. Inport_dimensions cannot contain value smaller than one!" << endlog();
            return;
        }
        N_INPORT_ENTRIES += INPORT_DIMENSIONS[i];
    }
    for(uint i = 0; i < N_OUTPORTS; i++) {
        if(OUTPORT_DIMENSIONS[i] < 1) {
            log(Error) << "ADIns::AddEncoderIns: Could not add EncoderIn. Outport_dimensions cannot contain value smaller than one!" << endlog();
            return;
        }
        N_OUTPORT_ENTRIES += OUTPORT_DIMENSIONS[i];
    }
	...
	// Check if the total number of entries matches the size of FROM_WHICH_INPORT and FROM_WHICH_ENTRY
    if ((FROM_WHICH_INPORT.size() != N_OUTPORT_ENTRIES) || (FROM_WHICH_ENTRY.size() != N_OUTPORT_ENTRIES)) {
        log(Error) << "ADIns::AddEncoderIns: Could not add EncoderIn. The number of entries in from_which_inport_E and from_which_entry_E should equal the total number of output values." << endlog();
        return;
    }
    ...
    for(uint j = 0; j < N_OUTPORT_ENTRIES; j++) {
        if( FROM_WHICH_INPORT[j] > (n_inports_E+N_INPORTS) || FROM_WHICH_INPORT[j] <= 0 ) {
            log(Error) << "ADIns::AddEncoderIns: Could not add EncoderIn. From_which_inport array contains port no. " << FROM_WHICH_INPORT[j] << " which does not exist according to inport_dimensions_E!" << endlog();
            return;
        }
        else if ( FROM_WHICH_ENTRY[j] > INPORT_DIMENSIONS[ FROM_WHICH_INPORT[j]-1-n_inports_E] || FROM_WHICH_ENTRY[j] <= 0 ) {
            log(Error) << "ADIns::AddEncoderIns: Could not add EncoderIn. From_which_entry array contains entry no. " << FROM_WHICH_ENTRY[j] << " which does not exist for inport no. " << FROM_WHICH_INPORT[j] << "!" << endlog();
            return;
        }
    }
    ...
    for( uint i = n_inports_E; i < (n_inports_E+N_INPORTS); i++ ) {
		addEventPort( "Ein"+to_string(i+1), inports_E[i] );
	}
    for( uint i = n_outports_E; i < (n_outports_E+N_OUTPORTS); i++ ) {
        addPort( "Eout"+to_string(i+1), outports_E[i] );
    }
    ...
    for( uint i = 0; i < n_inports_E; i++ ) {
        input_msgs_E[i].values.resize( inport_dimensions_E[i] );
	}
    ...
    log(Warning) << "ADIns::AddEncoderIns: Succesfully added EncoderIns with " << N_INPORTS << " inports and " << N_OUTPORTS << " outports!" << endlog();
